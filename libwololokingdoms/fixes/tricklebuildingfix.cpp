...
void trickleBuildingPatch(genie::DatFile* aocDat) {
  ...
  aocDat->UnitHeaders[newUnitsStartID + 8].TaskList.push_back(
      aocDat->UnitHeaders[sheepID].TaskList[0]);
  for (size_t i = 0; i < 4; i++) {
    aocDat->UnitHeaders[newUnitsStartID + i].TaskList.push_back(
        aocDat->UnitHeaders[sheepID].TaskList[0]);
    aocDat->UnitHeaders[newUnitsStartID + i].TaskList.push_back(
        aocDat->UnitHeaders[feitoriaID].TaskList[i]);
    aocDat->UnitHeaders[newUnitsStartID + i].TaskList[1].WorkValue1 = 1;
    aocDat->UnitHeaders[newUnitsStartID + 4 + i].TaskList.push_back(
        aocDat->UnitHeaders[feitoriaID].TaskList[i]);
    ...
    aocDat->UnitHeaders[newUnitsStartID + 9].TaskList.push_back(
        aocDat->UnitHeaders[feitoriaID].TaskList[i]);
...
DatPatch trickleBuildingFix = {&trickleBuildingPatch,
