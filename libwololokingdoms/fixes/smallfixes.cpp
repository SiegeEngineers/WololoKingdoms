...
void smallPatches(genie::DatFile* aocDat) {
  ...
  int16_t const mountains[] = {310,  311,  744,  745,  1041, 1042,
  ...
  // unpackable TC
  aocDat->UnitHeaders[PTWC].TaskList.push_back(
      aocDat->UnitHeaders[pTrebId].TaskList[0]);
  ...
  for (size_t civIndex = 0; civIndex < aocDat->Civs.size(); civIndex++) {
    ...
    aocDat->Civs[civIndex].Units[eliteCamelArcherID] =
        aocDat->Civs[0].Units[eliteCamelArcherID];
    aocDat->Civs[civIndex].Units[cannonGalleonID].Creatable.HeroMode -=
        128; // restore patrol for cannon galleons
    aocDat->Civs[civIndex].Units[eliteCannonGalleonID].Creatable.HeroMode -=
        128;
    aocDat->Civs[civIndex].Units[PTWC] =
        aocDat->Civs[0].Units[PTWC]; // unpackable TC
    aocDat->Civs[civIndex].Units[monkeyBoyID].Class =
        10; // This makes gaia monkey boys attack instead of being captured
    aocDat->Civs[civIndex].Units[sharkatzorID].HideInEditor =
        0; // unhide sharkatzor in editor
    aocDat->Civs[civIndex].Units[battleElephantID].Combat.BlastAttackLevel =
        10; // give battle elephants half-attack blast damage
    aocDat->Civs[civIndex]
        .Units[eliteBattleElephantID]
        .Combat.BlastAttackLevel = 10;
    // UP Siege Tower works better with 0.5 collision size
    aocDat->Civs[civIndex].Units[siegeTowerID].Size = {0.5, 0.5, 2};
    // Kings can't attack, to make sure there are no graphics issues we set the
    // attack graphic to the standing graphic
  ...
  aocDat->TechTree.ResearchConnections.erase(
      aocDat->TechTree.ResearchConnections.begin() + 6);
  ...
  // Make it possible for maps to go back to aoc water by disabling fire/demo
  // galley and a disabler for fire/demo shop
  aocDat->Effects[fireShipTechId].EffectCommands[0].TargetUnit = fireGalleyId;
  genie::EffectCommand effect =
      aocDat->Effects[fireShipTechId].EffectCommands[0];
  ...
  aocDat->Techs[FireShipDisablerId].RequiredTechs = {
      feudalAgeResearchId, -1, -1, -1, -1, -1};
  ...
  aocDat->Effects[FireShipDisablerTechId].EffectCommands[0].Amount =
      fireShipResearchId;
  ...
  aocDat->Effects[DemoShipDisablerTechId].EffectCommands[0].Amount =
      demoShipResearchId;
  ...
  aocDat->Effects[xPatchDisablingTechId].EffectCommands[0].Amount =
      xPatchResearchId;
  effect.Type = 3;
  ...
  aocDat->Techs[castleCoinageDisablingResearchId] =
      aocDat->Techs[FireShipDisablerId];
  aocDat->Techs[castleCoinageDisablingResearchId].Name =
      "Castle Coinage Disabler";
  aocDat->Techs[castleCoinageDisablingResearchId].EffectID =
      castleCoinageDisablingTechId;
  aocDat->Techs[impCoinageDisablingResearchId] =
      aocDat->Techs[FireShipDisablerId];
  aocDat->Techs[impCoinageDisablingResearchId].Name = "Imp Coinage Disabler";
  aocDat->Techs[impCoinageDisablingResearchId].EffectID =
      impCoinageDisablingTechId;
  aocDat->Techs[impBankingDisablingResearchId] =
      aocDat->Techs[FireShipDisablerId];
  aocDat->Techs[impBankingDisablingResearchId].Name = "Imp Banking Disabler";
  aocDat->Techs[impBankingDisablingResearchId].EffectID =
      impBankingDisablingTechId;
  ...
  aocDat->Techs[impBankingResearchId].RequiredTechs[2] =
      castleCoinageResearchId;
  ...
  aocDat->Effects[castleCoinageDisablingTechId].EffectCommands[0].Amount =
      castleCoinageResearchId;
  ...
  aocDat->Effects[impCoinageDisablingTechId].EffectCommands[0].Amount =
      impCoinageResearchId;
  ...
  aocDat->Effects[impBankingDisablingTechId].EffectCommands[0].Amount =
      impBankingResearchId;
  aocDat->Effects[impBankingDisablingTechId].EffectCommands.pop_back();
}

DatPatch smallFixes = {&smallPatches, "Several small civ bug fixes"};
